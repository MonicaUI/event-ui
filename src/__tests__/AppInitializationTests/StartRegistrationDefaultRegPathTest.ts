...
import {
  PAGE_TRANSITION_HIDE_LOADING,
  PAGE_TRANSITION_SHOW_LOADING
...
dialogContainer.pageTransitionShowLoading = jest.fn(() => {
  return { type: PAGE_TRANSITION_SHOW_LOADING };
});
dialogContainer.pageTransitionHideLoading = jest.fn(() => {
  return { type: PAGE_TRANSITION_HIDE_LOADING };
});

const wait = ms => new Promise(resolve => setTimeout(resolve, ms));

test('shows first reg page when coming in without reg type and calls correct page loading functions', async () => {
  ...
  await wait(0);
  expect(mainApp.findByText('Reg Path 1 Page 1')).toBeTruthy();
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  expect(require('../../clients/RegCartClient').prototype.createRegCart).toHaveBeenCalled();
  expect(dialogContainer.pageTransitionShowLoading).toHaveBeenCalled();
  expect(dialogContainer.pageTransitionHideLoading).toHaveBeenCalled();
});
