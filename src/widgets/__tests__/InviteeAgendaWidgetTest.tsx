...
import InviteeAgendaWidget, { getAllInvitees } from '../InviteeAgendaWidget';
import {
  eventRegsWithGroupLeader,
  eventRegsWithAttendee,
  eventRegsWithGroupLeaderAndAttendee
} from '../../testUtils/eventRegMocks';
import { shallow } from 'enzyme';
import { updateIn } from 'icepick';
import { dstInfo } from '../../../fixtures/EasternTimeDstInfoFixture';

const translate = resx => resx;
...
test('InviteeAgendaWidget produces props from state', () => {
  const store = {
    getState() {
      return state;
    },
  ...
  const widget = shallow(<InviteeAgendaWidget {...props} store={store} config={{}} />);

  // Hack to stop pretty-format from breaking on this object
  const expectedProps = updateIn(widget.props(), ['classes', 'sessionFilter', 'multiselect', 'menu'], x => ({ ...x }));

  expect(expectedProps).toMatchSnapshot();
});

test('InviteeAgendaWidget does not throw error when regcart is empty during logout or cancel', () => {
  const modifiedState = {
    ...state,
    registrationForm: {
      regCart: {
  ...
  const createWidget = () => {
    shallow(<InviteeAgendaWidget {...props} store={store} config={{}} />);
  };
  expect(createWidget).not.toThrowError();
});

test.skip('selected device timezone from timezone dialog', () => {
  const store = {
    getState() {
      return {
        ...state,
  ...
  const adjustedStartTime = widget.getElement().props.children.props.sessions[0].startTime;
  const adjustedEndTime = widget.getElement().props.children.props.sessions[0].endTime;
...
test.skip('selected event timezone from timezone dialog', () => {
  const store = {
    getState() {
      return {
        ...state,
...
describe('Guest Details for the Attendee Dropdown', () => {
  test('should return guests with Group Leader', () => {
    const regCart = {
      eventRegistrations: eventRegsWithGroupLeader
    };
    const allInvitees = getAllInvitees(regCart, translate, true);
    expect(allInvitees).toMatchSnapshot();
  });

  test('should return guests with Primary Attendee', () => {
    const regCart = {
      eventRegistrations: eventRegsWithAttendee
    };
    const allInvitees = getAllInvitees(regCart, translate, true);
    expect(allInvitees).toMatchSnapshot();
  });

  test('should return guests with Group Leader and not with Primary Attendee', () => {
    const regCart = {
      eventRegistrations: eventRegsWithGroupLeaderAndAttendee
    };
    const allInvitees = getAllInvitees(regCart, translate, true);
    expect(allInvitees).toMatchSnapshot();
  });
