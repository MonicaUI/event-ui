...
import SpeakersWidget, { recordSpeakerFileDownloadActivity } from '../Speakers/SpeakersWidget';
...
test('SpeakersWidget produces props from state', () => {
  const widget = shallow(<SpeakersWidget store={store} />);
  const props = updateIn(widget.props(), ['classes'], x => ({ ...x }));
  expect(props).toMatchSnapshot();
});

test('publishes activity fact for speaker document', async () => {
  const s = mockStore(store.getState());
  await s.dispatch(
  ...
  expect(mockPublishAttendeeActivityFact).toHaveBeenCalledWith('token', {
    ...
    fileName: 'bio',
    fileType: 'pdf',
    documentId: '0fe53ba6-a2bc-4282-9f2d-8ee4131c07f5'
  });
});

test('does not publish activity fact without speaker', async () => {
  const s = mockStore(store.getState());
  await s.dispatch(
    recordSpeakerFileDownloadActivity(null, {
      name: null,
      extension: null,
      id: null
    })
